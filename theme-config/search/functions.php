...
function get_myrows($id = null){
  ...
  $myrowsquery = "SELECT r.object_id
    FROM wp_uam_accessgroup_to_object q
    INNER JOIN wp_uam_accessgroup_to_object w
    ON q.group_id = w.group_id AND q.object_id = {$userid} OR w.group_id = 0
    RIGHT JOIN wp_uam_accessgroup_to_object r
    ON w.object_id = r.object_id
    INNER JOIN wp_posts t
    ON r.object_id = t.ID
...
function get_uam_groups_by_ID($id){
  global $wpdb;
  $groups = array();
  $results = $wpdb->get_results('SELECT * FROM '.$wpdb->prefix.'uam_accessgroup_to_object A INNER JOIN '.$wpdb->prefix.'uam_accessgroups B on A.group_id = B.ID WHERE object_id = '.$id);
  if($results!=null){
